...
def test_latent_sizes(latent_size, model_type, device):
    ...
    if model_type in ("grm", "gpcm"):
        ldgs = generate_ldgs(n_indicators, latent_size).to(device)
        ...
        if model_type != "normal":
            ldgs = generate_ldgs(n_indicators, latent_size, shrink = True).to(device)
    ...
    assert(invert_factors(iwave.loadings).add(-ldgs).abs().le(ABS_TOL).all())
    assert(iwave.intercepts.add(-ints).abs().le(ABS_TOL).all())
